...
var normalsShader=copyShader.clone();normalsShader.fragmentShader=normals_frag;normalsShader.uniforms={tSize:{type:"v2"},tPosition:{type:"t"},tFace1:{type:"t"},tFace2:{type:"t"},tFace3:{type:"t"}};var RESOLUTION,renderer,mesh,targetRT,normalsRT,originalRT,previousRT,positionRT,constraintsRT,facesRT,steps=60,tSize=new THREE.Vector2,scene=new THREE.Scene,camera$1=new THREE.Camera;
...
function update(){integrate();for(var a=0;a<steps;a++){updating()&&mouseOffset();for(var b=0;8>b;b++)solveConstraints(b)}computeVertexNormals()}
...
mesh$1;
function init$3(a){RESOLUTION$1=Math.ceil(Math.sqrt(vertices.length));var b=(new THREE.TextureLoader).load("../../assets/Leather_007_DISP.PNG");b=new THREE.MeshPhysicalMaterial({color:16767520,bumpMap:b,bumpScale:.35,metalness:.1,roughness:.6,clearCoat:.8,clearCoatRoughness:.35,dithering:!0});b.onBeforeCompile=function(a){a.uniforms.tPosition={value:positionRT.texture};a.uniforms.tNormal={value:normalsRT.texture};a.vertexShader="uniform sampler2D tPosition;\nuniform sampler2D tNormal;\n"+a.vertexShader;a.vertexShader=
