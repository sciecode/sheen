...
integrateShader.uniforms={dt2:{type:"f"},tSize:{type:"v2"},tOriginal:{type:"t"},tPrevious:{type:"t"},tPosition:{type:"t"}};var mouseShader=copyShader.clone();mouseShader.fragmentShader=mouse_frag;mouseShader.uniforms={psel:{value:null},tSize:{type:"v2"},mouse:{type:"v3"},tOriginal:{type:"t"},tPosition:{type:"t"}};var constraintsShader=copyShader.clone();constraintsShader.fragmentShader=constraints_frag;
constraintsShader.uniforms={cID:{value:null},tSize:{type:"v2"},tOriginal:{type:"t"},tPosition:{type:"t"},tConstraints:{type:"t"}};var normalsShader=copyShader.clone();normalsShader.fragmentShader=normals_frag;normalsShader.uniforms={tSize:{type:"v2"},tPosition:{type:"t"},tFace1:{type:"t"},tFace2:{type:"t"},tFace3:{type:"t"}};
var RESOLUTION,renderer,mesh,targetRT,normalsRT,originalRT,previousRT,positionRT,constraintsRT,facesRT,steps=60,tSize=new THREE.Vector2,scene=new THREE.Scene,camera$1=new THREE.Camera,clock=new THREE.Clock;
...
function update(){integrate();for(var a=0;a<steps;a++){updating()&&a+5<steps&&mouseOffset();for(var b=0;8>b;b++)solveConstraints(b)}computeVertexNormals()}
...
function init$5(){renderer$1=new THREE.WebGLRenderer({antialias:!0});renderer$1.setSize(window.innerWidth,window.innerHeight);renderer$1.setPixelRatio(window.devicePixelRatio);renderer$1.gammaOutput=!0;renderer$1.physicallyCorrectLights=!0;renderer$1.shadowMap.enabled=!0;renderer$1.shadowMap.type=THREE.PCFShadowMap;renderer$1.debug.checkShaderErrors=!0;console.log(renderer$1.capabilities);document.body.appendChild(renderer$1.domElement);scene$1=new THREE.Scene;scene$1.background=new THREE.Color(1184530);
camera$2=new THREE.PerspectiveCamera(60,window.innerWidth/window.innerHeight,1,1E4);camera$2.position.z=-350;camera$2.position.y=-50;camera$2.position.x=0;camera$2.lookAt(new THREE.Vector3);calculate();init(scene$1);init$4(scene$1);init$3(scene$1);init$1(camera$2);init$2(renderer$1);dispose();animate()}function animate(){requestAnimationFrame(animate);update$1();update();renderer$1.setRenderTarget(null);renderer$1.render(scene$1,camera$2)}
