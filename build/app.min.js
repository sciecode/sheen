...
function init$1(a){camera=a;window.addEventListener("mousemove",onMouseMove);window.addEventListener("mousedown",onMouseDown);window.addEventListener("mouseout",onMouseOut);window.addEventListener("mouseup",onMouseUp)}
...
function onMouseMove(a){mouse.x=a.pageX/window.innerWidth*2-1;mouse.y=2*-(a.pageY/window.innerHeight)+1}function onMouseDown(a){0==a.button&&(interacting=!0)}function onMouseUp(a){0==a.button&&(interacting=!1,psel=void 0)}function onMouseOut(){interacting=!1;psel=void 0}
...
var RESOLUTION,renderer,mesh,targetRT,normalsRT,originalRT,previousRT,positionRT,adjacentsRT,distancesRT,steps=50,tSize=new THREE.Vector2,scene=new THREE.Scene,camera$1=new THREE.Camera;
