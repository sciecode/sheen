...
function init$3(a){RESOLUTION$1=Math.ceil(Math.sqrt(vertices.length));var b=(new THREE.TextureLoader).load("./src/textures/bmpMap.png");b=new THREE.MeshPhysicalMaterial({color:16767520,bumpMap:b,bumpScale:.25,metalness:0,roughness:.9,sheen:new THREE.Color(0,0,1),dithering:!0});b.onBeforeCompile=function(a){a.uniforms.tPosition={value:positionRT.texture};a.uniforms.tNormal={value:normalsRT.texture};a.vertexShader="uniform sampler2D tPosition;\nuniform sampler2D tNormal;\n"+a.vertexShader;a.vertexShader=
