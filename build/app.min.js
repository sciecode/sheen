...
var RESOLUTION,renderer,mesh,targetRT,ntargetRT,normalsRT,originalRT,previousRT,positionRT,constraintsRT,facesRT,steps=60,tSize=new THREE.Vector2,scene=new THREE.Scene,camera$1=new THREE.Camera;
...
function integrate(){mesh.material=integrateShader;integrateShader.uniforms.tSize.value=tSize;integrateShader.uniforms.dt.value=.016;integrateShader.uniforms.tOriginal.value=originalRT.texture;integrateShader.uniforms.tPrevious.value=previousRT.texture;integrateShader.uniforms.tPosition.value=positionRT.texture;renderer.setRenderTarget(targetRT);renderer.render(scene,camera$1);var a=previousRT;previousRT=positionRT;positionRT=targetRT;targetRT=a}
...
e.addAttribute("uv",geometry.attributes.uv);mesh$1=new THREE.Mesh(e,b);mesh$1.customDepthMaterial=c;mesh$1.castShadow=!0;a.add(mesh$1)}var objects,clock=new THREE.Clock;
...
c,d,e,f,g]}function update$1(){var a=clock.getElapsedTime();if(1<a&&4>a)for(var b=0;b<objects.length;b++){var c=objects[b],d=objects[b].baseIntensity;var e=(a-1)/3;var f=1>(e/=.5)?.5*e*e*e:.5*((e-=2)*e*e+2);c.intensity=d*f}}var renderer$1,camera$2,scene$1;
